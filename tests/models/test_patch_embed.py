...
def test_conv_patch_embed_output_shape(img_size, patch_size, in_channels, embed_dim):
    patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size, in_channels=in_channels, embed_dim=embed_dim)
...
def test_conv_patch_embed_invalid_input_size(img_size, patch_size, in_channels, embed_dim):
    patch_embed = ConvPatchEmbed(img_size=img_size, patch_size=patch_size, in_channels=in_channels, embed_dim=embed_dim)
    x = torch.randn(2, in_channels, img_size, img_size)  # batch_size=2
